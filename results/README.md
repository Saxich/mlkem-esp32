...
- [ATTR/](ATTR/) — results of the experiment from the `attr-inline-basemuls-experiment` branch, where the `ntt`/`invntt` routines and the `zetas`/`zetas_inv` tables in [../components/mlkem/ntt.c](../components/mlkem/ntt.c) are placed into internal SRAM via the ESP-IDF `IRAM_ATTR` / `DRAM_ATTR` attributes; contains the raw benchmark log (`benchmark_with_attr.txt`) and a side-by-side comparison against the unmodified `-Os` build (`comparison_Os_ATTR`). The measured benefit was inconsistent across optimization variants, security levels and operations, so the modification is kept only on the branch and is not part of `main`.
