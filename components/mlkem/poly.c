/*
 * Polynomial operations — compression, decompression, encoding, and arithmetic
 *
 * Licensing: GPL-3.0-or-later (see LICENSE and CREDITS.md)
 *
 * SPEED_CODE functions: pq-crystals/kyber reference (CC0-1.0 OR Apache-2.0)
 *   https://github.com/pq-crystals/kyber
 *   Used under CC0-1.0.
 *
 * STACK_CODE functions: pq-code-package/mlkem-c-embedded (Apache-2.0 OR CC0-1.0)
 *   https://github.com/pq-code-package/mlkem-c-embedded
 *   Used under CC0-1.0.
 *
 * Additions:
 *   - cmp_poly_compress (SPEED_CODE): created to support cmp_indcpa_enc;
 *     inspired by the equivalent cmp_poly_compress from mlkem-c-embedded.
 *   - poly_basemul_acc_montgomery: created based on poly_basemul_montgomery
 *     from mlkem-c-embedded, extended with accumulation into an existing poly.
 *   - Performance-critical sections replaced with macros to guide compiler
 *     code generation and reduce function call overhead.
 *
...
**************************************************/
#define COMPRESS_COEFF4(t, a, i, j) do {                                    \
    int16_t u = a->coeffs[8 * i + j];                                      \
    u += (u >> 15) & MLKEM_Q;                                              \
    t[j] = ((((uint16_t)u << 4) + MLKEM_Q / 2) / MLKEM_Q) & 15;          \
} while(0)

#define COMPRESS_COEFF5(t, a, i, j) do {                                    \
    int16_t u = a->coeffs[8 * i + j];                                      \
    u += (u >> 15) & MLKEM_Q;                                              \
...
void poly_compress(uint8_t r[MLKEM_POLYCOMPRESSEDBYTES], const poly *a)
{
    ...
    for (i = 0; i < MLKEM_N / 8; i++) {
        COMPRESS_COEFF4(t, a, i, 0);
        COMPRESS_COEFF4(t, a, i, 1);
        COMPRESS_COEFF4(t, a, i, 2);
        COMPRESS_COEFF4(t, a, i, 3);
        COMPRESS_COEFF4(t, a, i, 4);
        COMPRESS_COEFF4(t, a, i, 5);
        COMPRESS_COEFF4(t, a, i, 6);
        COMPRESS_COEFF4(t, a, i, 7);
        r[0] = t[0] | (t[1] << 4);
        r[1] = t[2] | (t[3] << 4);
        r[2] = t[4] | (t[5] << 4);
        r[3] = t[6] | (t[7] << 4);
    ...
    for (i = 0; i < MLKEM_N / 8; i++) {
        COMPRESS_COEFF5(t, a, i, 0);
        COMPRESS_COEFF5(t, a, i, 1);
        COMPRESS_COEFF5(t, a, i, 2);
        COMPRESS_COEFF5(t, a, i, 3);
        COMPRESS_COEFF5(t, a, i, 4);
        COMPRESS_COEFF5(t, a, i, 5);
        COMPRESS_COEFF5(t, a, i, 6);
        COMPRESS_COEFF5(t, a, i, 7);
        r[0] = (t[0] >> 0) | (t[1] << 5);
...
#elif (MLKEM_POLYCOMPRESSEDBYTES == 160)
  // unsigned int j;  //prekladac tvrdi unused
  uint8_t t[8];
  for(i=0;i<MLKEM_N/8;i++) {
    t[0] = (a[0] >> 0);
    t[1] = (a[0] >> 5) | (a[1] << 3);
    ...
    // for(j=0;j<8;j++)
    //   r->coeffs[8*i+j] = ((uint32_t)(t[j] & 31)*MLKEM_Q + 16) >> 5;
    FROM_MSG_BIT(r, t, i, 0);
    FROM_MSG_BIT(r, t, i, 1);
    FROM_MSG_BIT(r, t, i, 2);
    FROM_MSG_BIT(r, t, i, 3);
    FROM_MSG_BIT(r, t, i, 4);
    FROM_MSG_BIT(r, t, i, 5);
    FROM_MSG_BIT(r, t, i, 6);
    FROM_MSG_BIT(r, t, i, 7);
  }
#else
#error "MLKEM_POLYCOMPRESSEDBYTES needs to be in {128, 160}"
#endif
...
#error "MLKEM_INDCPA_MSGBYTES must be equal to MLKEM_N/8 bytes!"
  ...
  //neoptimalizujem kvoli const-time
  for(i=0;i<MLKEM_N/8;i++) {
    for(j=0;j<8;j++) {
      r->coeffs[8*i+j] = 0;
...
// moje makro
#define TO_MSG_BIT(m, p, i, j) do {                             \
    uint16_t t = p->coeffs[8 * i + j];                         \
...
void poly_tomsg(uint8_t msg[MLKEM_INDCPA_MSGBYTES], const poly *a)
{
  ...
  for (i = 0; i < MLKEM_N / 8; i++) {
      msg[i] = 0;
      TO_MSG_BIT(msg, a, i, 0);
      TO_MSG_BIT(msg, a, i, 1);
      TO_MSG_BIT(msg, a, i, 2);
      TO_MSG_BIT(msg, a, i, 3);
      TO_MSG_BIT(msg, a, i, 4);
      TO_MSG_BIT(msg, a, i, 5);
      TO_MSG_BIT(msg, a, i, 6);
      TO_MSG_BIT(msg, a, i, 7);
  }
 ...
 *
 * Description: Multiplies two polynomials in the NTT domain using base-case
 *              multiplication (FIPS 203, Algorithm 11: MultiplyNTTs).
 *              Processes 128 pairs of coefficients, applying alternating
 *              zeta factors derived from the precomputed zetas table.
 *              Results are in Montgomery representation.
 *
...
void poly_basemul_montgomery(int16_t *r, const int16_t *a, const int16_t *b)
{
    ...
    for (i = 0; i < MLKEM_N; i += 16, zeta += 4) {
        basemul(r + i +  0, a + i +  0, b + i +  0,  zeta[0]);
        basemul(r + i +  2, a + i +  2, b + i +  2, -zeta[0]);
        basemul(r + i +  4, a + i +  4, b + i +  4,  zeta[1]);
        basemul(r + i +  6, a + i +  6, b + i +  6, -zeta[1]);
        basemul(r + i +  8, a + i +  8, b + i +  8,  zeta[2]);
        basemul(r + i + 10, a + i + 10, b + i + 10, -zeta[2]);
        basemul(r + i + 12, a + i + 12, b + i + 12,  zeta[3]);
        basemul(r + i + 14, a + i + 14, b + i + 14, -zeta[3]);
    }
}

/*************************************************
 * Name:        poly_basemul_acc_montgomery
 *
 * Description: Multiplies two polynomials in the NTT domain and accumulates
 *              the result into r (FIPS 203, Algorithm 11: MultiplyNTTs).
 *              Processes 128 pairs of coefficients, applying alternating
 *              zeta factors derived from the precomputed zetas table.
 *              Results are in Montgomery representation.
 *
 * Arguments:   - int16_t *r:        pointer to output polynomial to accumulate in
 *              - const int16_t *a:  pointer to first input polynomial
 *              - const int16_t *b:  pointer to second input polynomial
 **************************************************/
void poly_basemul_acc_montgomery(int16_t* r, const int16_t* a, const int16_t* b)
{
    const int16_t* zeta = zetas + 64;
    unsigned int i;
    for (i = 0; i < MLKEM_N; i += 16, zeta += 4) {
        basemul_acc(r + i +  0, a + i +  0, b + i +  0,  zeta[0]);
        basemul_acc(r + i +  2, a + i +  2, b + i +  2, -zeta[0]);
        basemul_acc(r + i +  4, a + i +  4, b + i +  4,  zeta[1]);
        basemul_acc(r + i +  6, a + i +  6, b + i +  6, -zeta[1]);
        basemul_acc(r + i +  8, a + i +  8, b + i +  8,  zeta[2]);
        basemul_acc(r + i + 10, a + i + 10, b + i + 10, -zeta[2]);
...
void poly_tomont(poly *r)
{
  unsigned int i;
  for(i=0;i<MLKEM_N;i++){
    int32_t t = XT_MULL(r->coeffs[i], MLKEM_F);
    r->coeffs[i] = MLKEM_MONT_RED(t);
  }
...
void poly_reduce(poly *r)
{
  ...
  for(i=0;i<MLKEM_N;i++)
    ...
    r->coeffs[i] = MLKEM_BARRETT_RED(r->coeffs[i]);
}
...
void poly_sub(poly *r, const poly *a, const poly *b)
{
  ...
  for(i=0;i<MLKEM_N;i++)
    ...
    *              - const poly *a: pointer to input polynomial
    **************************************************/
    int cmp_poly_compress(const uint8_t r[MLKEM_POLYCOMPRESSEDBYTES], const poly *a)
{
    unsigned char rc = 0;
    unsigned int i;
    uint8_t t[8];
        ...
        for (i = 0; i < MLKEM_N / 8; i++) {
            ...
            COMPRESS_COEFF4(t, a, i, 6);
            COMPRESS_COEFF4(t, a, i, 7);
            uint8_t b0 = t[0] | (t[1] << 4);
            uint8_t b1 = t[2] | (t[3] << 4);
            uint8_t b2 = t[4] | (t[5] << 4);
            uint8_t b3 = t[6] | (t[7] << 4);
            rc |= r[0] ^ b0;
            rc |= r[1] ^ b1;
            rc |= r[2] ^ b2;
            rc |= r[3] ^ b3;
        ...
        for (i = 0; i < MLKEM_N / 8; i++) {
            ...
            COMPRESS_COEFF5(t, a, i, 6);
            COMPRESS_COEFF5(t, a, i, 7);
            uint8_t b0 = (t[0] >> 0) | (t[1] << 5);
            uint8_t b1 = (t[1] >> 3) | (t[2] << 2) | (t[3] << 7);
            uint8_t b2 = (t[3] >> 1) | (t[4] << 4);
            uint8_t b3 = (t[4] >> 4) | (t[5] << 1) | (t[6] << 6);
            uint8_t b4 = (t[6] >> 2) | (t[7] << 3);
            rc |= r[0] ^ b0;
            rc |= r[1] ^ b1;
            rc |= r[2] ^ b2;
            rc |= r[3] ^ b3;
   ...
   // odstranene sizeof
    ...
    {
      uint8_t buf[MLKEM_ETA1*MLKEM_N/4]__attribute__((aligned(8)));
    ...
    **************************************************/
    ...
    {
      uint8_t buf[MLKEM_ETA2*MLKEM_N/4]__attribute__((aligned(8)));
    ...
    **************************************************/
    void poly_noise_eta1(poly *r, const unsigned char *seed, unsigned char nonce, int add) {
        unsigned char buf[MLKEM_ETA1 * MLKEM_N / 4]__attribute__((aligned(8)));
    ...
    *
    **************************************************/
    //split na 2 funckie sa nevyplati, testovane
