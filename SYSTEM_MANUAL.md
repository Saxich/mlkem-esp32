...
> **Note:** Always use `-Os` when running performance or memory benchmarks. `-Os` has been verified as the fastest option on ESP32 for this codebase — it produces tighter code that fits better in the instruction cache than `-O2`. Debug builds will report significantly worse cycle counts.
